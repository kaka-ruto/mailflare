...
export default function AccountDetailsPage() {
	...
	return <div className="space-y-6">
		...
		<section className="space-y-5 rounded-3xl bg-white p-6">
			...
			<label className="flex items-center gap-3 text-sm"><input type="checkbox" checked={account.canManageMailboxes} onChange={(event) => setAccount({ ...account, canManageMailboxes: event.target.checked })} />Allow this account to add and remove inboxes</label>
			<label className="flex items-center gap-3 text-sm"><input type="checkbox" checked={!account.disabled} onChange={(event) => setAccount({ ...account, disabled: !event.target.checked })} />Account enabled</label>
